...
# 20180921 - Added 7 columns to the output.
#sudo /bin/netstat -nutlp|egrep -v '(LISTENING|127.0.0.1|Active|Proto)'|awk '{print ","$NF,$1,","$4}' |sed -e 's/ /,/g' \
sudo /bin/netstat -nutlp|egrep -v '(LISTENING|127.0.0.1|Active|Proto)'|awk '{print ",,,,,,,"$NF,$1,","$4}' |sed -e 's/ /,/g' \
-e '/Xvnc/c\,,,,,,,Xvnc,tcp,VNC,5900,6000' -e '/splunkd/c\,,,,,,,SPLUNK,tcp,SPLUNK DEPLOYMENT MANAGER,8089,8089' \
-e '/sshd/c\,,,,,,,SSH,tcp,SSH,22,22' \
...
do  echo -e "$(hostname -I),$line"|sed  -e 's/[[:digit:]]\+\///'  -e 's/tcp/TCP/g' -e 's/udp/UDP/g'
